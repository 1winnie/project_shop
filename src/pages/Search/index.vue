...
export default {
  ...
  // 当组件挂载完毕之前才能执行一次，先于发请求之前
  beforeMount(){
      // Object.assign合并
      Object.assign(this.searchParams,this.$route.query,this.$route.params)
  },
  // 组件挂载完毕执行一次，根据带的参数不同要发多次请求
  mounted() {
  ...
  methods: {
    ...
    removeCategoryName(){
      // 把带给服务器的参数置空了，还需要向服务器发请求
        ...
        this.getData();
        // 地址栏也需要修改：进行路由跳转
        // 严谨：本意是删除query，如果路径当中出现params，则不应该删除，路由跳转的时候应该带着
    ...
    // 删除关键字
    removekeyword(){
        // 给服务器带有参数的searchParams的keyword置空
    ...
    // 自定义事件回调
    trademarkInfo(tradmark){
      this.searchParams.trademark = `${tradmark.tmId}`+":"+ `${tradmark.tmName}`;
      this.getData();
    },
    removetradmark(){
  ...
  watch:{
        ...
        $route(newValue,oldValue){
          ...
          // 再次发送ajax请求
          this.getData();
          // 每一次请求完毕，应该把1,2,3级分类置空，让它接收下一次的响应1,2,3id
